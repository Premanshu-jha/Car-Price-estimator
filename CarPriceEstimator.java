...
public class CarPriceEstimator {
	...
	public double getSalePrice(String makeAndModel, int yearManufactured, double milesDriven, 
		...
		if(!(airBagsCount>=2 && hasEbd && hasAbs)) salePrice = salePrice - 1000;
		...
		if(hasRearViewCamera && (hasSunRoof || hasAutoAC)) salePrice = salePrice + 500;
		...
		//     reduce price by $500
		if(hasAccidentHistory) salePrice = salePrice - 500;
		...
		if(additionalMiles>1000 && additionalMiles<=10000) salePrice = salePrice - 500;
		
		// b) if # miles over-driven is greater than 10000 AND less than or equal to 30000 then
		//        reduce sale price by 1000
		if(additionalMiles>10000 && additionalMiles<=30000) salePrice = salePrice - 1000;
		
		
		// c) if # miles over-driven is greater than 30000 then
		//        reduce sale price by 1500
		if(additionalMiles>30000) salePrice = salePrice - 1500;
